  ...
  >
    ...
    ════════════════════════════════════════════════════════ -->
      <header class="mb-8 text-center">
        <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight">
          Hola, <span class="text-indigo-600">{{ 'Juan' }}</span> 👋
        </h1>
    ...
    ════════════════════════════════════════════════════════ -->
      <section aria-label="Servicios prepago">
        ...
        -->
          <li v-for="(card, index) in prepaidCards" :key="index" role="listitem">
            <button
              type="button"
              class="w-full text-left rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md hover:border-indigo-300 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all duration-200 ease-in-out active:scale-95 p-5 group"
              :aria-label="`Comprar ${card.title} por ${card.price}`"
              @click="openModal(card)"
            >
              <!-- Icono / badge de categoría decorativo -->
              <div
                class="mb-3 inline-flex items-center justify-center w-10 h-10 rounded-lg bg-indigo-100 group-hover:bg-indigo-200 transition-colors duration-200"
              >
              ...
              <h3
                class="text-base font-semibold text-gray-900 mb-1 group-hover:text-indigo-700 transition-colors duration-150"
              >
                {{ card.title }}
              </h3>
              ...
              <div class="flex items-center justify-between">
                ...
                <span
                  class="text-xs text-gray-400 group-hover:text-indigo-400 transition-colors duration-150"
                >
                  Comprar →
                </span>
              </div>
          ...
          >
            ...
            >
              <div
                ...
                class="relative z-10 w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6 border border-gray-200"
                @keydown="trapFocus"
              >
                <!-- Icono decorativo -->
                <div
                  class="flex items-center justify-center w-12 h-12 mx-auto mb-4 rounded-full bg-indigo-100"
                >
                  <span class="text-indigo-600 text-2xl" aria-hidden="true">🛒</span>
                </div>

                <!-- Título del modal -->
                <h2 :id="modalTitleId" class="text-lg font-bold text-gray-900 text-center mb-2">
                  Confirmar compra
                </h2>
                ...
                <div class="flex gap-3">
                  ...
                  <button
                    ref="cancelBtnRef"
                    type="button"
                    class="flex-1 rounded-xl border border-gray-300 bg-white text-gray-700 py-2.5 text-sm font-semibold hover:bg-gray-50 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 active:scale-95 transition-all duration-150"
                    @click="closeModal"
                  >
                    Cancelar
                  </button>
                  ...
                  <button
                    ref="confirmBtnRef"
                    type="button"
                    class="flex-1 rounded-xl bg-indigo-600 text-white py-2.5 text-sm font-semibold hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 active:scale-95 transition-all duration-150"
